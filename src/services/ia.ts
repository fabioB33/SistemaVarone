...
export async function analizarConIA(texto: string): Promise<RespuestaIA> {
  ...
  try {
    ...
    logger.error(`[IA] Error en llamada a ${ENV.AI_PROVIDER}:`, errorMsg);
  ...
  if (ENV.LOG_LEVEL === 'debug') {
    logger.info('[IA] Respuesta raw:', respuestaRaw.substring(0, 300));
  }
  ...
  try {
    ...
    logger.error('[IA] Error parseando respuesta:', respuestaRaw.substring(0, 200));
